...
## What each agent needs (paste checklist)
...
3. Open Chat 3 (Agent 3 — Tech writer) — optional per feature,
   ...
   → Paste AGENT_3_TECHWRITER.md + updated AGENT_LOG.md
     + Agent 1 code + Agent 2 findings
...
The code is wrong if:
  ...
  → Paste the failure into Chat 1 (Agent 1) with:
    "This test failed — here's the error. Review your
...
**Chat 1 (Backend):** Keep alive for the whole feature. Start a new
session only when moving to a genuinely different feature. Before
starting fresh, paste the old session's key decisions into AGENT_LOG.md.
...
│       ├── auth.md
│       └── games.md
├── agent-prompts/            ← store the prompt templates here
│   ├── AGENT_1_BACKEND.md
│   ├── AGENT_2_QA.md
